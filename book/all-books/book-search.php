...
<h3 class="px-3 py-1 bg-white" >Showing results for "<em><?php echo $_GET['query'];?></em>"</h3>

<?php 
require "../../core/_dbconnect.php";
$sql = "SELECT * FROM `all-books` WHERE name like '%".$_GET['query']."%' or writer like '%".$_GET['query']."%' or type like '%".$_GET['query']."%'";
...
if ($total != 0){
    ...
    while ($a <= $total) {
        ...
        <h5 class="card-title">'.$result[name].'</h5>
        <p class="card-text">'.$result[writer].'</p>
        <a href="../../'.$result[link].'" class="more">More</a>
        </div></div>';
        $a++;
        }
    }
